...
- *ExportImages*. In this stage, all the processed rasters must be stored as parquet in HDFS. When this function is executed, you can export the images to local file system, by specifying a path where the image set is going to exported to. To do this, it creates a new image set, respecting the original file format, and replaces the new raster information. The resulting image set can be found in a new directory named as "sipt".
